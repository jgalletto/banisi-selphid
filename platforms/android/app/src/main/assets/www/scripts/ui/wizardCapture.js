...
/**
 * Launches the plugin that encapsulates the native Widget
 *
 * @method showWidget
 * @param String identifier The identifier of the user to be authenticated
 * @param String name The name of the user to be authenticated
 */
 ...
 */
function onSuccessWizardCapture(result) {
 ...
 * @param String result Object with response data
 */
function onErrorWizardCapture(result) {
    ...
    if (result != null && result) {
        ...
        if (result.responseText)
            ...
            switch (parseInt(data['finishStatus'])) {
                ...
                case WidgetLnPFinishStatus.Error: // Error
                    if (data['errorType']) {
                        if (parseInt(data['errorType']) === WidgetSelphIDErrorType.UnknownError) // Unknown Error
                        {
                            showErrorUI(fphi_str_unknown_error);
